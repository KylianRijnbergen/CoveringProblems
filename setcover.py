...
def main():
    ...
    #Objective function
    model.objective = minimize(xsum(x[lid] for lid, location in enumerate(problem.locations)))
    
    #Constraints
    for customer in problem.arrests:
        model += 1 <= xsum(aijs[customer.id - 1][lid] * x[lid] for lid, location in enumerate(problem.locations))
